...
contract TurnstileBond is TurnstileUser, ERC1155 {
    ...
    function cancel(uint256 _tokenId) external payable {
        harvest(_tokenId);
        require(msg.sender == bondInfo[_tokenId].seller, "!seller");
        require(bondInfo[_tokenId].raised <= bondInfo[_tokenId].minGoal, "minGoal passed");
        require(!bondInfo[_tokenId].canceled, "canceled");
    ...
    function claim(uint256 _tokenId) external {
        ...
        if(bondInfo[_tokenId].canceled) {
            ...
            (bool success, ) = msg.sender.call{value : amount * bondInfo[_tokenId].accrued / (bondInfo[_tokenId].raised + amount)}("");
            require(success, "refund failed");
        }
    }
